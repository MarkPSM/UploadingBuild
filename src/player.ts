...
async function svgToImageSource(
  svgPath: string,
  width: number,
  height: number
): Promise<ex.ImageSource> {
  return new Promise((resolve, reject) => {
    fetch(svgPath)
      ...
      .then((svgText) => {
        ...
        const img = new Image();
        img.onload = () => {
          // Desenhar em um canvas no tamanho desejado
          ...
          // Transformar o canvas em ImageSource do Excalibur
          const dataUrl = canvas.toDataURL("image/png");
          resolve(new ImageSource(dataUrl));
        };
        img.onerror = reject;
        img.src = url;
      })
...
const imageSource = await svgToImageSource(logo, 64, 64);
